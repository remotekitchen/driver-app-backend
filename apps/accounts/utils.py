...
def update_driver_work_history(driver, delivery):
    ...
    work_history, created = DriverWorkHistory.objects.get_or_create(
        ...
        defaults={
            ...
            "total_active_hours": Decimal("0.00"),
            "online_duration": Decimal("0.00"),
        },
    )
    ...
    if (
        delivery.est_delivery_completed_time and
        delivery.actual_delivery_completed_time <= delivery.est_delivery_completed_time
    ):
    ...
    if last_session:
        ...
        hours_online = Decimal(str(time_online.total_seconds() / 3600))
        work_history.online_duration = F("online_duration") + hours_online

    # Save updates
    work_history.save(update_fields=[
        "total_deliveries",
        "total_earnings",
        "on_time_deliveries",
        "online_duration",
